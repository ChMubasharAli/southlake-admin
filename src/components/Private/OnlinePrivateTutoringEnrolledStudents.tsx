...
interface OnlineTutoringForm {
  registrationId: number;
  parentFirstName: string;
  parentLastName: string;
  OnlineTutoringForms: Array<Record<string, any>>; // Nested array for additional data
}

const OnlinePrivateTutoringEnrolledStudents: React.FC = () => {
  const [searchQuery, setSearchQuery] = useState<string>("");
  const [data, setData] = useState<OnlineTutoringForm[]>([]);
  const [filteredData, setFilteredData] = useState<OnlineTutoringForm[]>([]);

  // Fetch Data
  const fetchData = useCallback(async () => {
    try {
      const response = await axios.get(
        "https://southlakebackend.onrender.com/api/getOnlinePrivParent"
      );
      if (response.data) {
  ...
  const handleExcelDownload = () => {
    const excelData = filteredData.map((form) => ({
      ...
      "Program Bought": form.OnlineTutoringForms.length || " ",
    }));
  ...
  const handleDownload = (registrationId: number) => {
    ...
    const doc = new jsPDF();
    doc.setFontSize(18);
    doc.text("Online Private Tutoring Form Details", 10, 10);
    doc.setFontSize(12);
    ...
    <div className="p-6 lg:max-w-[80%] mx-auto ">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-bold text-[#1A3D16] ">
          Online Private Tutoring Enrolled Students
        </h2>
            ...
            {filteredData.length > 0 ? (
              filteredData.map((form) => (
                ...
                >
                  ...
                  <td className="py-3 px-6 text-left">
                    {form.OnlineTutoringForms.length}
                  </td>
...
export default OnlinePrivateTutoringEnrolledStudents;
